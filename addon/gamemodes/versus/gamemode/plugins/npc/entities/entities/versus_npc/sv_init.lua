...
function ENT:Initialize()
  ...
  if (not self.versusNpcData) then
    return
  end

  if (self.versusNpcData.OnSpawn) then
    self.versusNpcData:OnSpawn(self)
  end
end
...
function ENT:SetupNPC(npc)
  ...
  local health = npc.health or 100

  if (health ~= PLUGIN.NO_HEALTH) then
...
function ENT:Think()
  ...
  if (self.versusNpcData.OnThink) then
    self.versusNpcData:OnThink(self)
  end
...
function ENT:KeyValue(key, value)
  ...
  if (key == "npcid") then
    self:SetupNPC(PLUGIN.get(value))
  end
end
