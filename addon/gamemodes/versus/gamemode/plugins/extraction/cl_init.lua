...
function PLUGIN.updateConditionIndicators()
  if not IsValid(PLUGIN.assignedExtractionPoint) then return end
  ...
  local extractionPointName = PLUGIN.assignedExtractionPoint:GetExtractionName()
  ...
  for _, condition in ipairs(ents.FindByClass("versus_extraction_condition")) do
    if IsValid(condition) and condition:GetExtractionPointName() == extractionPointName then
      local completed = PLUGIN.hasCompletedCondition(LocalPlayer(), condition)
      ...
      if not completed then
        ...
        PLUGIN.conditionIndicators[condition:EntIndex()] = indicatorID
      end
    end
  end
end
